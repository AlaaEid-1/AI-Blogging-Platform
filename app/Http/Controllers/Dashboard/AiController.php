...
Do not return any markdown formatting around the JSON block. Return ONLY valid JSON.";
        ...
        try {
            ...
            if (! $text) {
                Log::error('Gemini API returned no text content: '.$response->body());
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                Log::error('Gemini invalid JSON: '.json_last_error_msg().' Text: '.$text);
            ...
            Log::error('Gemini API Exception: '.$e->getMessage().' in '.$e->getFile().':'.$e->getLine());
            report($e);
